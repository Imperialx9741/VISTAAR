...
import {
  getPlatformFeeRule,
  publishPlatformFeeRule,
  submitPlatformFeeRuleForReview,
...
import styles from "./PlatformFeeRuleDetailPage.module.css";
...
export function PlatformFeeRuleDetailPage({ ruleId }: { ruleId: string }) {
  const [profile, setProfile] = useState<AdminProfile | null>(null);
  const [profileFailed, setProfileFailed] = useState(false);

  const [rule, setRule] = useState<PlatformFeeRule | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadRuleOnMount() {
      try {
        const data = await getPlatformFeeRule(ruleId);
  ...
  async function runAction(
    label: string,
    action: () => Promise<PlatformFeeRule>,
  ) {
    setIsWorking(true);
    setActionError(null);
  ...
  return (
    ...
    >
      <Link href="/fare-management/platform-fee" className={styles.backLink}>
        ← Back to Platform Fee Management
      </Link>

      {loading && (
        <div className={styles.stateMessage} role="status" aria-live="polite">
          Loading platform fee rule…
        </div>
      )}
      ...
      {!loading && !loadError && rule && (
        <>
          <div className={styles.summaryCard}>
            ...
            <div>
              <div className={styles.summaryLabel}>Fee amount</div>
              <div className={styles.summaryValue}>
                {CURRENCY.format(rule.fee_amount)}
              </div>
            </div>
            <div>
                ...
                <button
                  ...
                  onClick={() =>
                    runAction("Submitted for review.", () =>
                      submitPlatformFeeRuleForReview(ruleId),
                    )
                  }
                >
                  Submit for review
          ...
          {showPublishField && (
            <div className={styles.reasonRow}>
              ...
              <button
                ...
                disabled={isWorking}
                onClick={() =>
                  runAction("Platform fee rule published.", () =>
                    publishPlatformFeeRule(
                      ruleId,
                      effectiveFrom
