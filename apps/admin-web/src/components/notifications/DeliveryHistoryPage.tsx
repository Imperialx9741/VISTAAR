"use client";

import { useCallback, useEffect, useState } from "react";
import Link from "next/link";
import { AppShell } from "@/components/layout/AppShell";
import { Pill } from "@/components/dashboard/Pill";
import { getAdminProfile } from "@/lib/api/dashboard";
import { searchDeliveries } from "@/lib/api/notifications";
import { ApiError, NoSessionError } from "@/lib/api/client";
import { toneForStatus } from "@/lib/status-tone";
import type { AdminProfile, Delivery } from "@/lib/api/types";
import styles from "./DeliveryHistoryPage.module.css";

const PAGE_SIZE = 20;
const CHANNELS = ["", "IN_APP", "SMS", "PUSH", "WHATSAPP"];
const STATUSES = ["", "PENDING", "SENT", "FAILED"];

const DATE_TIME = new Intl.DateTimeFormat("en-IN", {
  day: "numeric",
  month: "short",
  hour: "2-digit",
  minute: "2-digit",
});

function errorMessage(error: unknown): string {
  if (error instanceof NoSessionError) return error.message;
  if (error instanceof ApiError) {
    if (error.code === "FORBIDDEN") {
      return "Your admin account does not have access to Notifications.";
    }
    return error.message;
  }
  return "Couldn't load notification history. Try again.";
}

interface ListState {
  deliveries: Delivery[];
  totalPages: number;
  page: number;
}

export function DeliveryHistoryPage() {
  const [profile, setProfile] = useState<AdminProfile | null>(null);
  const [profileFailed, setProfileFailed] = useState(false);

  const [channel, setChannel] = useState("");
  const [status, setStatus] = useState("");
  const [list, setList] = useState<ListState | null>(null);
  const [listLoading, setListLoading] = useState(true);
  const [listError, setListError] = useState<string | null>(null);

  const loadList = useCallback(
    async (searchChannel: string, searchStatus: string, page: number) => {
      setListLoading(true);
      setListError(null);
      try {
        const result = await searchDeliveries(
          searchChannel,
          searchStatus,
          page,
          PAGE_SIZE,
        );
        setList({
          deliveries: result.items,
          totalPages: Math.max(1, result.pagination.total_pages),
          page: result.pagination.page,
        });
      } catch (error) {
        setListError(errorMessage(error));
      } finally {
        setListLoading(false);
      }
    },
    [],
  );

  useEffect(() => {
    let ignore = false;

    async function loadProfileOnMount() {
      try {
        const result = await getAdminProfile();
        if (!ignore) setProfile(result);
      } catch {
        if (!ignore) setProfileFailed(true);
      }
    }

    async function loadListOnMount() {
      try {
        const result = await searchDeliveries("", "", 1, PAGE_SIZE);
        if (ignore) return;
        setList({
          deliveries: result.items,
          totalPages: Math.max(1, result.pagination.total_pages),
          page: result.pagination.page,
        });
      } catch (error) {
        if (!ignore) setListError(errorMessage(error));
      } finally {
        if (!ignore) setListLoading(false);
      }
    }

    loadProfileOnMount();
    loadListOnMount();
    return () => {
      ignore = true;
    };
  }, []);

  if (profileFailed) {
    return (
      <div className={styles.fullPageMessage}>
        <p>Something went wrong loading Notifications. Try reloading.</p>
      </div>
    );
  }

  if (!profile) {
    return (
      <div className={styles.fullPageMessage} role="status" aria-live="polite">
        <p>Loading Notifications…</p>
      </div>
    );
  }

  return (
    <AppShell
      title="Notifications"
      subtitle="Delivery history across every channel"
      adminRole={profile.role}
      adminPermissions={profile.permissions}
    >
      <div className={styles.headerRow}>
        <div className={styles.sectionLabel}>Delivery history</div>
        <div className={styles.headerLinks}>
          <Link href="/notifications/templates" className={styles.viewLink}>
            Manage templates →
          </Link>
          <Link href="/notifications/broadcasts" className={styles.viewLink}>
            Broadcasts →
          </Link>
        </div>
      </div>

      <form
        className={styles.searchBar}
        onSubmit={(event) => event.preventDefault()}
      >
        <select
          value={channel}
          onChange={(event) => {
            setChannel(event.target.value);
            void loadList(event.target.value, status, 1);
          }}
          className={styles.statusSelect}
          aria-label="Filter by channel"
        >
          {CHANNELS.map((value) => (
            <option key={value} value={value}>
              {value === "" ? "Any channel" : value}
            </option>
          ))}
        </select>
        <select
          value={status}
          onChange={(event) => {
            setStatus(event.target.value);
            void loadList(channel, event.target.value, 1);
          }}
          className={styles.statusSelect}
          aria-label="Filter by delivery status"
        >
          {STATUSES.map((value) => (
            <option key={value} value={value}>
              {value === "" ? "Any status" : value}
            </option>
          ))}
        </select>
      </form>

      {listLoading && (
        <div className={styles.stateMessage} role="status" aria-live="polite">
          Loading delivery history…
        </div>
      )}

      {!listLoading && listError && (
        <div className={styles.errorBanner} role="alert">
          {listError}
        </div>
      )}

      {!listLoading && !listError && list && (
        <>
          <table className={styles.table}>
            <thead>
              <tr>
                <th>Recipient</th>
                <th>Channel</th>
                <th>Template</th>
                <th>Status</th>
                <th>Sent</th>
              </tr>
            </thead>
            <tbody>
              {list.deliveries.length === 0 && (
                <tr>
                  <td colSpan={5} className={styles.emptyCell}>
                    No deliveries found.
                  </td>
                </tr>
              )}
              {list.deliveries.map((delivery) => (
                <tr key={delivery.delivery_id}>
                  <td className={styles.idCell}>{delivery.user_id}</td>
                  <td>{delivery.channel}</td>
                  <td>{delivery.template_key}</td>
                  <td>
                    <Pill tone={toneForStatus(delivery.status)}>
                      {delivery.status}
                    </Pill>
                  </td>
                  <td>{DATE_TIME.format(new Date(delivery.created_at))}</td>
                </tr>
              ))}
            </tbody>
          </table>

          <div className={styles.pagination}>
            <button
              type="button"
              className={styles.secondaryButton}
              onClick={() => loadList(channel, status, list.page - 1)}
              disabled={list.page <= 1}
            >
              Previous
            </button>
            <span className={styles.pageIndicator}>
              Page {list.page} of {list.totalPages}
            </span>
            <button
              type="button"
              className={styles.secondaryButton}
              onClick={() => loadList(channel, status, list.page + 1)}
              disabled={list.page >= list.totalPages}
            >
              Next
            </button>
          </div>
        </>
      )}
    </AppShell>
  );
}
